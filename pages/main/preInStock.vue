	...
	<u-col span="10">
		...
		<view class="search-group">
			<view class="input-row border">
				<u-field v-model="id" label="送货清单号" placeholder="请输入送货清单号">
				</u-field>
			</view>
			<view class="input-row border">
				<u-cell-group>
		...
		<view>
			<button type="primary" size="default" @click="getPlanList">查询</button>
		</view>
<view>
			查询结果
		...
		<!--<u-table :align="align" :borderColor="borderColor" :style="{
			...
			<template v-for="item in unDolist ">
				<u-tr><u-td  width="100%">送货清单号:{{ item.id }} 供应商:{{item.gysname}} {{item.createTime.substr(0,10)}}</u-td></u-tr>
		...
		<template v-for="item in unDolist ">
			<view>
				<view>
					{{ item.id }}<u-line color="red" direction="col" hair-line="1px" length="30rpx" />{{item.gysname}}
				</view>
				<view>
					<u-collapse>
