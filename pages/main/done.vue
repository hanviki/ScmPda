<template>
	...
	<view class="search-group">
			...
			<view class="input-row border" v-show="IsDownUp == 1">
				<u-cell-group>
					<u-field @click="showPicker" v-model="werkst" :disabled="true" label="院区" placeholder="请选择院区"
			...
			<view class="input-row border" v-show="IsDownUp == 1">
				<u-cell-group>
					<u-field @click="showPickerLgort" v-model="lgortName" :disabled="true" label="库房" placeholder="请选择库房"
					 right-icon="arrow-down-fill">
					</u-field>
				</u-cell-group>
...
<view>
		...
		<!---<table :align="align" :borderColor="borderColor" :style="{
			display:'block' 
		}">
			<u-tr>
				<u-td width="40%">药品名称</u-td>
				<u-td width="20%">待收数量</u-td>
				<u-td width="20%">供应日期</u-td>
				<u-td width="20%">计划编号</u-td>
			</u-tr>
			<u-tr v-for="item in unDolist" :key="item.id">
				<u-td width="40%">{{ item.txz01 }}</u-td>
				<u-td width="20%">{{ item.gMenge }}</u-td>
				<u-td width="20%">{{ item.createTime.substr(0,10) }}</u-td>
				<u-td width="20%">{{ item.id }}</u-td>
			</u-tr>
		</u-table>-->
		...
		methods: {
			...
			getAreaList: function() { //第一次回去数据
				this.$request.TokenRequest({
					...
					this.werksList.push({id:'-1',name:'全部',code:'0'});
					this.lgortList=[];
				}).catch((e) => {
					console.log(e)
				});
			...
			getPlanList: function() { //第一次回去数据
				...
				this.loading = true;
				this.$request.TokenRequest({
					url: 'viewSupplyplan/doneStatus/1', //待
					method: 'get'
